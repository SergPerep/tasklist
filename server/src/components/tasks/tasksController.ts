...
const task_update_put = async (req: Request, res: Response) => {
    try {
        ...
        if (!description) {
            ...
            const isUpdateSuccessful = changeTaskStatus.rowCount != null ? changeTaskStatus.rowCount > 0 : false;
            ...
            const isUpdateSuccessful = editTask.rowCount != null ? editTask.rowCount > 0 : false;
            if (!isUpdateSuccessful) return res.status(404).json({ messageToUser: "No such task related to this user" });
        }
